...
class User_Forms_Shortcodes{
	...
	public static function shortcode_login_form_call( $atts, $content = "" ) {
		...
		if( !is_user_logged_in() ) { 
			...
			$styles = (!empty($args['color']) ? '<style>.userforms-login #wp-submit{background:'.$args['color'].'</style>' : '');
			
			return '<div class="userforms-login">'.$styles.wp_login_form( $args ).$links.'</div>';
	...
	private static function register_form($args, $options){
		...
		$register_form_top = apply_filters( 'register_form_top', '', $args );
				...
				case 'first_name':
					if( $args['first'] ) { 
				 		$custom_fields .= sprintf(
							'<div class="uf-field uf-field-text %1$s">
								...
								<input type="text" name="%1$s" autocomplete="first_name" class="input %1$s" placeholder="%2$s" size="20" %3$s />
							</div>',
							$type,
						...
						if( $options['fields']['required'][$index] ){
							$javascript .= 'if( jQuery(".input.'.$type.'").val() == "") { jQuery(".userforms-register .status .error").append( "'.esc_html( $options['fields']['title'][$index] ).' is required field<br>" ); isvalid = false;}';
						}
					}
			 		break;
			 	...
			 	case 'last_name':
			 		if( $args['last'] ) { 
				 		$custom_fields .= sprintf(
							'<div class="uf-field uf-field-text %1$s">
								<label for="%1$s">%2$s %4$s</label>
								<input type="text" name="%1$s" autocomplete="last_name" class="input %1$s" placeholder="%2$s" size="20" %3$s />
							</div>',
							$type,
							esc_html( $options['fields']['title'][$index] ),
							( ( $options['fields']['required'][$index] ) ? $required_attr : '' ),
			 	...
			 	case 'log':
			 		if( $args['useroremail'] ) {
				 		$custom_fields .= sprintf(
							'<div class="uf-field uf-field-text %1$s">
								<label for="%1$s">%2$s %4$s</label>
								<input type="text" name="%1$s" autocomplete="username" class="input %1$s" placeholder="%2$s" size="20" %3$s />
							</div>',
							$type,
							esc_html( $options['fields']['title'][$index] ),
							( ( $options['fields']['required'][$index] ) ? $required_attr : '' ),
			 	...
			 	case 'email':
			 		$custom_fields .= sprintf(
						'<div class="uf-field uf-field-text %1$s">
							<label for="%1$s">%2$s %4$s</label>
							<input type="email" name="%1$s" autocomplete="email" class="input %1$s" placeholder="%2$s" size="20" %3$s />
						</div>',
						$type,
						esc_html( $options['fields']['title'][$index] ),
						( ( $options['fields']['required'][$index] ) ? $required_attr : '' ),
					...
					$javascript .= 'if( jQuery(".input.'.$type.'").val() == "") { jQuery(".userforms-register .status .error").append( "'.esc_html( $options['fields']['title'][$index] ).' is required field<br>" ); isvalid = false;}';
					$javascript .= 'var regEx = /\S+@\S+\.\S+/;';
					$javascript .= 'var validEmail = regEx.test(jQuery(".input.'.$type.'").val());';
					$javascript .= 'if( !validEmail ) { jQuery(".userforms-register .status .error").append( "'.esc_html( $options['fields']['title'][$index] ).' is email field<br>" ); isvalid = false;}';
			 		break;
			 	...
			 	case 'pwd':
			 		$custom_fields .= sprintf(
						'<div class="uf-field uf-field-text %1$s">
							<label for="%1$s">%2$s %4$s</label>
							<input type="password" name="%1$s" autocomplete="current-password" placeholder="%2$s" minlength="5" spellcheck="false" class="input %1$s" size="20" %3$s />
						</div>',
						$type,
						esc_html( $options['fields']['title'][$index] ),
						( ( $options['fields']['required'][$index] ) ? $required_attr : '' ),
			 	...
			 	case 'cpwd':
			 		if( $args['confirm'] ) {
				 		$custom_fields .= sprintf(
							'<div class="uf-field uf-field-text %1$s">
								<label for="%1$s">%2$s %4$s</label>
								<input type="password" name="%1$s" autocomplete="confirm-password" placeholder="%2$s" spellcheck="false" class="input %1$s" size="20" %3$s />
							</div>',
							$type,
							esc_html( $options['fields']['title'][$index] ),
							( ( $options['fields']['required'][$index] ) ? $required_attr : '' ),
						...
						$javascript .= 'if( jQuery(".input.'.$type.'").val() != jQuery(".input.pwd").val() ) { jQuery(".userforms-register .status .error").append( "'.esc_html( $options['fields']['title'][$index] ).' is not matched<br>" ); isvalid = false;}';
				 	}
			 		break;
			 	...
			 	case 'text':
			 		$custom_fields .= sprintf(
						'<div class="uf-field uf-field-text %1$s">
							<label for="%1$s">%2$s %4$s</label>
							<input type="text" name="%1$s" class="input %1$s" placeholder="%2$s" size="20" %3$s />
						</div>',
						$slug,
						esc_html( $options['fields']['title'][$index] ),
						( ( $options['fields']['required'][$index] ) ? $required_attr : '' ),
					...
					if( $options['fields']['required'][$index] ){
						$javascript .= 'if( jQuery(".input.'.$slug.'").val() == "") { jQuery(".userforms-register .status .error").append( "'.esc_html( $options['fields']['title'][$index] ).' is required field<br>" ); isvalid = false;}';
					}
			 		break;
			 	...
			 	case 'number':
			 		$custom_fields .= sprintf(
						'<div class="uf-field uf-field-number %1$s">
							<label for="%1$s">%2$s %4$s</label>
							<input type="number" name="%1$s" class="number %1$s" placeholder="%2$s" size="20" %3$s />
						</div>',
						$slug,
						esc_html( $options['fields']['title'][$index] ),
						( ( $options['fields']['required'][$index] ) ? $required_attr : '' ),
					...
					if( $options['fields']['required'][$index] ){
						$javascript .= 'if( jQuery(".number.'.$slug.'").val() == "") { jQuery(".userforms-register .status .error").append( "'.esc_html( $options['fields']['title'][$index] ).' is required field<br>" ); isvalid = false;}';
					}
			 		break;
			 	...
			 	case 'textarea':
			 		$custom_fields .= sprintf(
						'<div class="uf-field uf-field-textarea %1$s">
							<label for="%1$s">%2$s %4$s</label>
							<textarea name="%1$s" class="textarea %1$s" placeholder="%2$s" rows="4" cols="50" %3$s></textarea>
						</div>',
						$slug,
						esc_html( $options['fields']['title'][$index] ),
						( ( $options['fields']['required'][$index] ) ? $required_attr : '' ),
					...
					if( $options['fields']['required'][$index] ){
						$javascript .= 'if( jQuery(".textarea.'.$slug.'").val() == "") { jQuery(".userforms-register .status .error").append( "'.esc_html( $options['fields']['title'][$index] ).' is required field<br>" ); isvalid = false;}';
					}
			 		break;
			 	...
			 	case 'select':
			 		...
			 		$custom_fields .= sprintf(
						'<div class="uf-field uf-field-select %1$s">
							...
							<select name="%1$s" class="select %1$s" %4$s>%3$s</select>
						</div>',
						$slug,
						esc_html( $options['fields']['title'][$index] ),
						$opt_str,
					...
					if( $options['fields']['required'][$index] ){
						$javascript .= 'if( jQuery(".select.'.$slug.'").val() == "") { jQuery(".userforms-register .status .error").append( "'.esc_html( $options['fields']['title'][$index] ).' is required field<br>" ); isvalid = false;}';
					}
			 		break;
			 	...
			 	case 'checkbox':
			 		...
			 		$custom_fields .= sprintf(
						'<div class="uf-field uf-field-checkbox %1$s">
							<label for="%1$s">%2$s %4$s</label>
					...
					if( $options['fields']['required'][$index] ){
						$javascript .= 'if( !jQuery(".uf-field-checkbox.'.$slug.' input").is(":checked") ) { jQuery(".userforms-register .status .error").append( "'.esc_html( $options['fields']['title'][$index] ).' is required field<br>" ); isvalid = false;}';
					}
			 		break;
			 	...
			 	case 'radio':
			 		...
			 		$custom_fields .= sprintf(
						'<div class="uf-field uf-field-radio %1$s">
							<label for="%1$s">%2$s %4$s</label>
					...
					if( $options['fields']['required'][$index] ){
						$javascript .= 'if( !jQuery(".uf-field-radio.'.$slug.' input").is(":checked") ) { jQuery(".userforms-register .status .error").append( "'.esc_html( $options['fields']['title'][$index] ).' is required field<br>" ); isvalid = false;}';
					}
			 		break;
		...
		if($javascript != ""){
			$javascript = '<script type="text/javascript">function UF_Reg_validate(){ isvalid = true;jQuery(".userforms-register .status").html( "<div class=\"error\"></div>" );'.$javascript.' if(isvalid){return true;}else{return false};}</script>';
		}
	...
	public static function shortcode_forgot_password_form_call( $atts, $content = "" ) {
		...
		switch ( $action ) {
			...
			case 'rp':
				...
				if ( isset( $_GET['key'] ) && isset( $_GET['login'] ) ) {
					...
					if( $keyerror != 0 ){
						...
						$form = "<script>document.location.href = '".$keyerror."';</script>";
					}else{
						$form =
							sprintf(
				...
				$styles = (!empty($args['color']) ? '<style>.userforms-retrive-password #wp-submit{background:'.$args['color'].'</style>' : '');
				break;
			...
			default:
				...
				$styles = (!empty($args['color']) ? '<style>.userforms-forgot-password #wp-submit{background:'.$args['color'].'</style>' : '');
		...
		if( !is_user_logged_in() ) { 
			...
			return '<div class="userforms-'.$pageform.'-password">'.$styles.$status.$form.$links.'</div>';
	...
	public function register_ajax_call_callback() {
		...
		$register_data = isset( $_REQUEST['register_data'] ) ? (wp_unslash($_REQUEST['register_data'])) : '';
		$response = array( 'success' => false );

		parse_str($register_data, $register_data_array);

		$reqarr = explode("&redirect_to=", sanitize_url(wp_unslash($_REQUEST['register_data'])));
	    ...
	    if ( ! is_wp_error( $new_user_id ) ) {
	    	...
	    	foreach ($register_data_array as $field => $fieldvalue) {
	    		...
	    		if( !in_array($field, array('role', 'redirect_to', 'email', 'log', 'pwd', 'cpwd', 'first_name', 'last_name')) ){
	    			...
	    			$customfield[] = $field;
	    			update_user_meta( $new_user_id, $field, $fieldvalue );
	    		}
	    	}
	...
	public function login_ajax_call_callback() {
		...
		$login_data = isset( $_REQUEST['login_data'] ) ? (wp_unslash($_REQUEST['login_data'])) : '';
		$response = array( 'success' => false );

		parse_str($login_data, $login_data_array);
		
		$reqarr = explode("&redirect_to=", sanitize_url(wp_unslash($_REQUEST['login_data'])));
		$reqarr = explode("&", $reqarr[1]);
		$redirect_to = esc_url(urldecode($reqarr[0]));
	...
	public function forgot_password_ajax_call_callback() {
		...
		$forgot_data = isset( $_REQUEST['forgot_data'] ) ? (wp_unslash($_REQUEST['forgot_data'])) : '';
		$response = array( 'success' => false );

		parse_str($forgot_data, $forgot_data_array);

		$reqarr = explode("&redirect_to=", sanitize_url(wp_unslash($_REQUEST['forgot_data'])));
		$reqarr = explode("&", $reqarr[1]);
		$redirect_to = esc_url(urldecode($reqarr[0]));
